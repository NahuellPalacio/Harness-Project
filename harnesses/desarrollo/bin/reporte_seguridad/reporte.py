...
def generar_md(resumen):
    ...
    partes += [
        ...
        _tabla([("Estándar", texto(k.get("standard"))), ("Versión", texto(k.get("version"))),
                ...
                ("SHA-256 del original", texto(k.get("sha256"))),
                ("Canal", texto(k.get("channel"))),
                ("Aceptó", texto(k.get("acceptedBy"))),
                ("Cuándo", texto(k.get("acceptedAt"))),
                ("Versión que esperaba el registro", texto(k.get("registryVersion")))],
               ("Campo", "Valor")),
        "",
    ]
    if k.get("blocking"):
...
def _pagina_alcance(resumen):
    ...
    filas += [("Generado", _f(resumen, "generatedAt")), ("Tarea", _f(resumen, "taskId")),
              ...
              ("SHA-256 del original", _f(resumen, "knowledge.sha256")),
              ("Canal", _f(resumen, "knowledge.channel")),
              ("Aceptó", _f(resumen, "knowledge.acceptedBy")),
              ("Cuándo", _f(resumen, "knowledge.acceptedAt")),
              ("Versión que esperaba el registro", _f(resumen, "knowledge.registryVersion")),
